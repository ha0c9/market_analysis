...
def heuristic_report(
    *,
    ...
    if opps:
        ...
        if any(
            token in f"{row.thesis} {row.angle}"
            for row in opps
            for token in ("指甲", "修甲", "情绪脉冲")
        ):
            limitations.append("长文产品细节被交易成消费品线索，与基本面无关，开盘后常见冲高回落，需用价格验证")
    ...
    *,
    ...
    prompt = {
        ...
        "instructions": (
            ...
            "明星长文不要只写成影视传媒偏空：先看故事细节。"
            "写到磨指甲/剪指甲时，散户会交易指甲刀产品（张小泉有指甲剪业务），这不是谐音梗，也不是代言/订单，开盘后常见冲高回落。"
